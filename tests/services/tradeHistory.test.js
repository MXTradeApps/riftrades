 ...
 * table's queue in order. Saving a trade queries `rift_trades` more than once — the
...
/** A `rift_trades` table holding `count` rows, none of them tombstoned. */
...
describe('saveTradeToHistory', () => {
  ...
  test('allows unnamed saves for Confirm Trade', async () => {
    ...
    const chains = mockTables({
      rift_trades: [{ data: saved, error: null }, existingTrades(1)],
      rift_entitlements: asFree(),
    });
    ...
    expect(chains.rift_trades[0].insert).toHaveBeenCalledWith([
  ...
  test('inserts the trade and returns saved data on success', async () => {
    ...
    expect(supabase.from).toHaveBeenCalledWith('rift_trades');
    expect(chains.rift_trades[0].insert).toHaveBeenCalledWith([
      expect.objectContaining({
        user_id: 'user-42',
        name: 'My Trade',
  ...
  test('mints a client_id so mobile can address the row', async () => {
    asUser();
    const chains = mockTables({
      rift_trades: [{ data: { id: 't1' }, error: null }, existingTrades(1)],
      rift_entitlements: asFree(),
    });

    await saveTradeToHistory('My Trade', [], [], totals);

    const [[[inserted]]] = chains.rift_trades[0].insert.mock.calls;
  ...
  test('propagates a database error', async () => {
    asUser();
    mockTables({ rift_trades: [{ data: null, error: { message: 'boom' } }], rift_entitlements: asFree() });
    const { data, error } = await saveTradeToHistory('My Trade', [], [], totals);
    expect(data).toBeNull();
  ...
  describe('free-tier trade window', () => {
    test('rolls the oldest trades off once the window is full', async () => {
      ...
      const sweep = chains.rift_trades[2];
    ...
    test('trims a history that arrived overfull back to the window', async () => {
      ...
      expect(chains.rift_trades[2].in.mock.calls[0][1]).toHaveLength(5);
    });

    test('leaves a Pro account alone', async () => {
      asUser();
      const chains = mockTables({
        rift_trades: [{ data: { id: 'newest' }, error: null }],
        rift_entitlements: asPro(),
      });
      ...
      expect(trimmed).toBe(0);
      expect(chains.rift_trades).toHaveLength(1);
    });
    ...
    test('does not trim on an entitlement read the app could not complete', async () => {
      ...
      const chains = mockTables({
        rift_trades: [{ data: { id: 'newest' }, error: null }],
        rift_entitlements: [{ data: null, error: { message: 'offline' } }],
      });
      ...
      expect(trimmed).toBe(0);
      expect(chains.rift_trades.at(-1).update).not.toHaveBeenCalled();
    });
  });
});
...
describe('getUserTrades', () => {
  ...
  test('returns the list of trades scoped to the user', async () => {
    ...
    const chains = mockTables({ rift_trades: [{ data: trades, error: null }] });

    const { data, error } = await getUserTrades();

    expect(error).toBeNull();
    expect(data).toEqual(trades);
    expect(chains.rift_trades[0].eq).toHaveBeenCalledWith('user_id', 'user-9');
    expect(chains.rift_trades[0].order).toHaveBeenCalledWith('created_at', { ascending: false });
  });

  test('hides trades tombstoned by another device', async () => {
    asUser();
    const chains = mockTables({ rift_trades: [{ data: [], error: null }] });

    await getUserTrades();

    expect(chains.rift_trades[0].is).toHaveBeenCalledWith('deleted_at', null);
  });
});
...
describe('getTradeById', () => {
  test('fetches a single trade by id and user', async () => {
    asUser('user-3');
    const chains = mockTables({ rift_trades: [{ data: { id: 'xyz' }, error: null }] });

    const { data } = await getTradeById('xyz');

    expect(data).toEqual({ id: 'xyz' });
    expect(chains.rift_trades[0].eq).toHaveBeenCalledWith('id', 'xyz');
    expect(chains.rift_trades[0].eq).toHaveBeenCalledWith('user_id', 'user-3');
  });
});
...
describe('updateTrade', () => {
  test('applies updates and stamps updated_at', async () => {
    asUser();
    const chains = mockTables({ rift_trades: [{ data: { id: 'u1', name: 'New' }, error: null }] });

    const { data, error } = await updateTrade('u1', { name: 'New' });

    expect(error).toBeNull();
    expect(data).toEqual({ id: 'u1', name: 'New' });
    expect(chains.rift_trades[0].update).toHaveBeenCalledWith(
...
describe('deleteTrade', () => {
  test('tombstones the trade rather than removing it', async () => {
    asUser('user-7');
    const chains = mockTables({ rift_trades: [{ error: null }] });

    const { data, error } = await deleteTrade('d1');

    expect(error).toBeNull();
    expect(data).toEqual({ success: true });
    expect(chains.rift_trades[0].delete).not.toHaveBeenCalled();
    expect(chains.rift_trades[0].update).toHaveBeenCalledWith(
      expect.objectContaining({ deleted_at: expect.any(String), updated_at: expect.any(String) })
    );
    expect(chains.rift_trades[0].eq).toHaveBeenCalledWith('id', 'd1');
    expect(chains.rift_trades[0].eq).toHaveBeenCalledWith('user_id', 'user-7');
  });

  test('returns the error when deletion fails', async () => {
    asUser();
    mockTables({ rift_trades: [{ error: { message: 'nope' } }] });
    const { data, error } = await deleteTrade('d1');
    expect(data).toBeNull();
